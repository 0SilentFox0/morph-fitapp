...
describe('useProgramDraftForm', () => {
  ...
  it('submit (edit) updates the program and goes back', async () => {
    ...
    expect(updateProgram).toHaveBeenCalledWith(
      'p1',
      expect.objectContaining({ name: 'Legs', tag: 'HIIT' })
    );
