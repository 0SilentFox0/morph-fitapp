import {
  buildParticipants,
  participantIdForName,
  programMeta,
} from '../../utils/training/program';
...
describe('programMeta', () => {
  it('uses the exercise count when exercises are present', () => {
    ...
    ).toBe('Strength · 2 exercises');
  });

  it('falls back to videoCount when exercises are absent', () => {
    expect(programMeta({ tag: 'Cardio', videoCount: 5 })).toBe(
      'Cardio · 5 exercises'
    );
  });
});
...
describe('buildParticipants', () => {
  it('creates ids for new names', () => {
    expect(buildParticipants(['Anna Lee'])).toEqual([
      { id: 'p-anna-lee', name: 'Anna Lee' },
    ]);
  });
