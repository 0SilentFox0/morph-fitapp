...
describe('useDraftProgramStore', () => {
  ...
  it('addExercises de-duplicates by id', () => {
    ...
    expect(useDraftProgramStore.getState().exercises.map((e) => e.id)).toEqual([
      1, 2, 3,
    ]);
  });
  ...
  it('addSet clones the last set, falling back to 20kg x 10 when there are none', () => {
    useDraftProgramStore
      .getState()
      .addExercises([exercise(1, [{ weight: 50, reps: 5 }])]);
  ...
  it('removeSet keeps at least one set', () => {
    useDraftProgramStore
      .getState()
      .addExercises([exercise(1, [{ weight: 20, reps: 10 }])]);
  ...
  it('updateSet merges a partial update into the targeted set', () => {
    ...
    expect(useDraftProgramStore.getState().exercises[0]!.sets[0]).toEqual({
      weight: 99,
      reps: 10,
    });
  });
