...
describe('useTrainersStore', () => {
  it('getTrainer returns a trainer by id', () => {
    expect(useTrainersStore.getState().getTrainer('t1')?.name).toBe(
      'Marcus Reed'
    );
  ...
  it('visibleTrainers applies the shared specialty and online-only filters', () => {
    ...
    expect(
      useTrainersStore
        .getState()
        .visibleTrainers('')
        .every((t) => t.specialties.includes('Yoga'))
    ).toBe(true);
  ...
  it('connect moves a trainer from none → pending and is idempotent afterwards', () => {
    ...
    expect(useTrainersStore.getState().getTrainer('t1')?.connection).toBe(
      'pending'
    );
