...
const PROGRAMS = mockTrainingPrograms.filter(
  (p) => p.exercises && p.exercises.length > 0
);
...
export function ClientProfileScreen() {
  ...
  const setActiveParticipant = useActiveTrainingStore(
    (s) => s.setActiveParticipant
  );
  ...
  const activeClient =
    participants.find((c) => c.participantId === activeParticipantId) ??
    participants[0] ??
    null;
  ...
  const selectedProgram =
    PROGRAMS.find(
      (p) => p.id === (selectedProgramId ?? activeClient.programId)
    ) ?? PROGRAMS[0]!;
  ...
  const assigned = mockTrainingPrograms.find(
    (p) => p.id === activeClient.programId
  );
  ...
  const subtitle = currentExercise
    ? `${currentExercise.name} · set ${activeClient.setIndex + 1}` +
      (activeClient.rest.running
        ? ` · rest ${formatClock(activeClient.rest.remainingSec)}`
        : '')
    : undefined;
            ...
            <TouchableOpacity
              onPress={() =>
                navigation.navigate('ClientsProfileExtended', {
                  clientId: activeClient.participantId,
                })
              }
            >
              ...
              >
                <Text
                  style={[
                    ...
                    active && styles.programTabTextActive,
                  ]}
                >
                  Program {i + 1}
                </Text>
