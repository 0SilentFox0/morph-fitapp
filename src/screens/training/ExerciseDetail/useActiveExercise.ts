...
export function useActiveExercise(params: RouteParams) {
  ...
  const toggleRepToFailure = useActiveTrainingStore(
    (s) => s.toggleRepToFailure
  );
  ...
  return {
    ...
    onWeightChange: (weight: number) =>
      updateSet(pid, exercise.id, setIndex, { weight }),
    onRepsChange: (reps: number) =>
      updateSet(pid, exercise.id, setIndex, { reps }),
    onToggleFailure: () => toggleRepToFailure(pid, exercise.id, setIndex),
    onStartRest: () =>
      startRest(pid, (currentSet.note && REST_SECONDS[currentSet.note]) || 60),
