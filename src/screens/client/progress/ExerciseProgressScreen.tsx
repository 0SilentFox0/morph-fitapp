...
export function ExerciseProgressScreen() {
  ...
  const exercises = listExerciseProgress(getCurrentUserHistory(), exerciseCatalog);
    ...
    <View style={styles.container}>
      ...
      {exercises.length === 0 ? (
        <View style={styles.emptyWrap}>
          <EmptyState icon="barbell-outline" title="No exercises yet" subtitle="Log a session to track per-exercise progress." />
        </View>
      ) : (
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
          {exercises.map((ex) => (
            <TouchableOpacity
              ...
              style={styles.card}
              activeOpacity={0.8}
              onPress={() => navigation.navigate('ExerciseProgressDetail', { exerciseId: ex.exerciseId })}
            >
...
const styles = StyleSheet.create({
  ...
  rm: { fontSize: typography.sizes.base, color: colors.accent, fontWeight: typography.weights.bold },
});
