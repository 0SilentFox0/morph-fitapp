          ...
          <>
            <View style={styles.hero}>
              <View
                style={[
                  styles.heroBadge,
                  { borderColor: tierColor(overview.league.key) },
                ]}
              >
                <Ionicons
                  name={tierIcon(overview.league.key)}
                  size={44}
                  color={tierColor(overview.league.key)}
                />
              </View>
            ...
            <View style={styles.statsRow}>
              <View style={styles.statCard}>
                ...
                <Text style={styles.statValue}>
                  {Math.round(overview.consistencyScore * 100)}%
                </Text>
              ...
              <View style={styles.statCard}>
                ...
                <Text style={styles.statValue}>
                  Top {topPercent(overview.strengthScore)}%
                </Text>
            ...
            <Text style={styles.note}>
              Consistency (regular, long-term training) counts most; verified
              strength vs everyone else adds a bonus.
            </Text>
              ...
              {[...LEAGUE_TIERS].reverse().map((tier) => {
                ...
                return (
                  <View
                    ...
                    style={[
                      styles.ladderRow,
                      current && styles.ladderRowCurrent,
                    ]}
                  >
                    ...
                    <View style={styles.ladderText}>
                      <Text
                        style={[
                          styles.ladderName,
                          current && styles.ladderNameCurrent,
                        ]}
                      >
                        {tier.name}
                      </Text>
                      <Text style={styles.ladderBand}>
                        ...
                        {tier.maxPercentile < 1
                          ? `–${Math.max(1, Math.round((1 - tier.maxPercentile) * 100))}%`
                          : ''}
                      </Text>
                    </View>
                    {current && (
...
const tierColor = (key: string) =>
  LEAGUE_TIERS.find((t) => t.key === key)?.color ?? colors.accent;
