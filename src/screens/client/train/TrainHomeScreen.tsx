...
export function TrainHomeScreen() {
  ...
  const assigned = sessions.filter(
    (s) => s.status === 'pending' && !!s.programId
  );
    ...
    <View style={styles.container}>
      ...
      >
        ...
        {PROGRAMS.map((p) => (
          ...
          >
            <Card style={styles.programCard}>
              ...
              <View style={styles.tagsRow}>
                <Tag
                  label={`${p.exercises?.length ?? 0} exercises`}
                  variant="default"
                />
        ...
        <Text style={styles.hint}>
          Pick exercises and start a custom session.
        </Text>
        <Button
