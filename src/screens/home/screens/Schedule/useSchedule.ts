...
export function useSchedule(onOpenSession: (s: Session) => void) {
  ...
  const monthDays = React.useMemo(() => {
    ...
    for (let i = 0; i < startPad; i++) cells.push({ dateKey: '', date: 0, empty: true });
    for (let d = 1; d <= total; d++) {
      cells.push({ dateKey: new Date(year, month, d).toISOString().slice(0, 10), date: d, empty: false });
    }
    return cells;
  }, [monthStart]);
  ...
  const monthCellSize = Math.floor((width - spacing.lg * 2 - spacing.xs * 6) / 7);
