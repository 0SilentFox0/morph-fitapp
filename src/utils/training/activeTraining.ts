...
export function seedParticipant(
  ...
  const exercises = (program.exercises ?? []).map((e) => ({
    ...e,
    sets: e.sets.map((s) => ({ ...s })),
  }));
...
export function deriveActiveGroup(
  ...
  const pendingToday = sessions.filter(
  ...
  const withExercises = programs.filter(
