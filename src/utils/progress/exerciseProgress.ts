...
export function listExerciseProgress(
  ...
  for (const training of history) {
    for (const logged of training.exercises) {
      const prev = byId.get(logged.exerciseId) ?? {
        exerciseId: logged.exerciseId,
        name:
          catalog[logged.exerciseId]?.name ?? `Exercise ${logged.exerciseId}`,
        sessions: 0,
        topWeight: 0,
        best1RM: 0,
...
export function exerciseSessionSeries(
  ...
  for (const training of history) {
    ...
    points.push({
      date: training.date,
      value: sessionValue(logged.sets, metric),
    });
  }
...
export function overallVolumeSeries(
  history: CompletedTraining[]
): ProgressPoint[] {
