...
**Модуль:** Programs · **Phase:** 1 · **Файлів-сусідів:** `programs.md` (TBD) (technical)
...
- API: `programs.md` (TBD) § `GET /programs`, `POST /programs`, `GET /programs/{id}`, `PATCH /programs/{id}`, `DELETE /programs/{id}`, `POST /programs/{id}/archive`, `POST /programs/{id}/like`
...
- API: `programs.md` (TBD) § `POST /programs/{id}/assign`, `GET /programs/{id}/assignments`, `DELETE /client-programs/{id}`, `GET /me/programs`, `GET /me/programs/{client_program_id}`
